...
(function () {
  ...
  function buildQuiz(box) {
    ...
    var id = box.getAttribute('data-quiz-id') || 'check';
    var order, idx, correct, answered;
    var ui = el('div', 'qz');
    box.appendChild(ui);
    ...
    function renderQuestion() {
      ...
      order.forEach(function (_, i) {
        var d = el('i', 'qz-dot' + (i < idx ? (answered[i] ? ' ok' : ' no') : (i === idx ? ' now' : '')));
        dots.appendChild(d);
      });
      ...
      if (q.type === 'mc' || q.type === 'multi') {
        var items = q.options.map(function (o, i) { return { text: o, i: i }; });
        items = shuffle(items);
        items.forEach(function (it, k) {
          ...
          lab.appendChild(inp);
          lab.appendChild(el('span', 'qz-mark'));
          lab.appendChild(el('span', 'qz-opt-text', it.text));
          form.appendChild(lab); opts.push(lab);
        });
      } else if (q.type === 'tf') {
      ...
      var check = el('button', 'btn btn-primary qz-check', 'Prüfen');
      check.type = 'submit';
    ...
    function renderSummary() {
      ...
      var prev = load(progressKey(id));
      if (!prev || ratio > prev.ratio) store(progressKey(id), { ratio: ratio, date: new Date().toISOString().slice(0, 10) });
      updateLernpfad();
      ui.innerHTML = '';
      var s = el('div', 'qz-sum');
      var big = el('div', 'qz-score', '<span>' + correct + '</span><small>/ ' + questions.length + '</small>');
      s.appendChild(big);
      var msg = ratio >= 0.8 ? 'Sehr gut – dieses Kapitel sitzt.' : ratio >= 0.5 ? 'Solide Basis. Schau dir die Erklärungen zu den offenen Punkten noch einmal an.' : 'Noch Luft nach oben – die Verständnisbox oben hilft beim zweiten Anlauf.';
      s.appendChild(el('p', 'qz-msg', msg));
      var list = el('ol', 'qz-review');
      order.forEach(function (qi, i) {
        var q = questions[qi];
        list.appendChild(el('li', answered[i] ? 'ok' : 'no', '<span class="qz-rv-q">' + q.q + '</span>'));
      });
      s.appendChild(list);
      var again = el('button', 'btn btn-primary', 'Nochmal mischen <span class="arr" aria-hidden="true">↻</span>');
      again.type = 'button';
    ...
    function draw() {
      ...
      if (disc >= -1e-9) {
        ...
        [l1, l2].forEach(function (l) {
          ...
          var n = Math.hypot(v[0], v[1]) || 1; v = [v[0] / n * R * 1.4, v[1] / n * R * 1.4];
  ...
  function odeExplorable(host) {
    ...
    function draw() {
      ...
      // Lösungskurve per Runge-Kutta in beide Richtungen
  ...
  /* ---------- Formeln überall im Lernbereich setzen ---------- */
